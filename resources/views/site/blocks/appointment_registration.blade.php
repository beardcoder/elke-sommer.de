...
<section id="registration_form">
  <div class="container mx-auto my-6 max-w-3xl">
    ...
    <div class="space-y-4 p-6 sm:p-8 md:space-y-6">
      ...
      @else
        <form class="space-y-4 md:space-y-6" id="{{ $formId }}" action="{{ route('appointment.registration') }}"
          method="POST" x-data x-validate x-on:submit="$validate.submit">
          @csrf
          <x-honeypot />

          <label for="appointment" class="mb-2 block text-sm font-medium text-gray-900">Termin wählen *</label>
          <select id="appointment" name="appointment" required data-error-msg="Bitte einen Termin auswählen"
          ...
          <div>
            ...
            <input type="email" name="email" id="email"
              ...
              placeholder="meine@mail.de" data-error-msg="Bitte eine gültige E-Mail Adresse eingeben" x-validate.email
              required>
          </div>
            ...
            <div class="flex flex-row flex-wrap">
              <label class="order-2 text-sm" for="privacy">
                ...
                <a target="_blank" href="{{ route('frontend.page', [$block->getRelated('privacy')[0]->slug]) }}"
                  class="text-primary-600 underline">Datenschatzerklärung</a>
                <span>gelesen *</span>
              </label>
          ...
          @endif
          <button type="submit" x-bind:disabled="!$validate.isComplete('{{ $formId }}')"
